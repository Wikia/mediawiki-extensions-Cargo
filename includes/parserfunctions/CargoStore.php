...
class CargoStore {
	...
	public static function storeAllData( $title, $tableName, $tableFieldValues, $tableSchema ) {
		...
		// Fandom-start: original code was moved up and modified to support AUTO_INCREMENT.
		...
		$rowAlreadyExists = self::doesRowAlreadyExist( $cdb, $title, $tableName, $tableFieldValues, $tableSchema );
		if ( $rowAlreadyExists ) {
			// Fandom-start: end started atomic before returning from function
			$cdb->endAtomic( __METHOD__ );
			// Fandom-end
			return;
		}
		...
		// Fandom-start: use autoincrement ID value if not set
		if ( $curRowID == null ) {
		...
		foreach ( $fieldTableFieldValues as $tableNameAndValues ) {
			...
			// Fandom-start: Update _rowID if using AUTO_INCREMENT.
